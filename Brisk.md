...
| **Traders marketing page** | Same admin + public routes with `pageSlug=traders` · `GET /testimonials?type=trader` for reviews | Sections: `trader_hero`, `trader_benefits` (4 items), `trader_workflow` (6 steps), `professional_potential` (heading), `reputation_engine` + `growth_matrix` (journey-style cards + items), `trader_cta`; full page includes `page.id`, `seo`, section `type` alias |
