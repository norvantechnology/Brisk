  ...
  definition: {
    ...
    info: {
      ...
      description: [
        ...
        '- **Customer / Jobs:** /jobs. **Customer / Checkout:** /invoices, /payments, /bookings. **Realtime:** GET /realtime + Socket.IO /socket.io',
        '',
    ...
    tags: [
      ...
      { name: 'Mobile / Auth', description: 'Customer & Trader auth. Responses include `nextStep` (e.g. `VERIFY_OTP` for traders dual mobile+email OTP, `VERIFY_PHONE` for customers, `TRADER_ONBOARDING`, `TRADER_HOME`). Traders verify both OTPs via POST /auth/verify-otp on the same screen. Forgot password (2 screens): POST /auth/forgot-password (email|mobile → same OTP) then POST /auth/reset-password (code + newPassword + confirmPassword).' },
      {
        ...
        description: [
          ...
          '',
          '**Prerequisites:** `POST /auth/register` (role=TRADER) → `POST /auth/verify-otp` with mobileCode + emailCode (mock mobile `123456`; email = dynamic 6-digit code from inbox).',
          '',
          '**Figma flow:** Business Verification → Sole/Company Verification → Document Verification → Submit',
          '',
          '**Auth:** Bearer access token on every endpoint. Mobile mock OTP: `123456`. Email OTP: use the dynamic code from the verification email.',
        ].join('\n'),
      },
...
const SWAGGER_CSS = `
  ...
  .swagger-ui .opblock .opblock-summary-method span,
  .swagger-ui .opblock .opblock-summary-method * {
    color: #ffffff !important;
    -webkit-text-fill-color: #ffffff !important;
  }
  ...
  /* Final contrast overrides — must win over earlier broad * rules */
  .swagger-ui .opblock .opblock-summary-method,
  .swagger-ui .opblock .opblock-summary-method span {
    color: #ffffff !important;
    -webkit-text-fill-color: #ffffff !important;
  }
  .swagger-ui .opblock.opblock-get .opblock-summary-method { background-color: #2563eb !important; }
  .swagger-ui .opblock.opblock-post .opblock-summary-method { background-color: #16a34a !important; }
  .swagger-ui .opblock.opblock-put .opblock-summary-method { background-color: #ca8a04 !important; }
