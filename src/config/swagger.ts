  ...
  definition: {
    ...
    tags: [
      ...
      },
      {
        name: 'Customer / Jobs',
        description: [
          'Post a New Job + Choose Location + Publish (Direct Trader).',
          '',
          '**Auth:** Customer Bearer.',
          '',
          '**UI flow after Accept Offer:**',
          '1. Post a New Job — use jobFormConfig for quote type / min-max budget show-hide; upload images via POST /uploads purpose=job_photo then POST /jobs with photoUrls.',
          '2. Choose Location — POST /addresses (create) then PUT /jobs/{id}/location (select).',
          '3. Publish — POST /jobs/{id}/publish → navigate to Payment Details (data.invoice).',
          '',
          '**Direct Trader (offer applied):** quoteType locked FIXED, budget hidden, serviceCharge required at publish.',
          '',
      ...
      },
      {
        ...
        description: [
          'Payment Details → Checkout → Success / Fail screens.',
          '',
          '**Auth:** Customer Bearer (must own booking/invoice/payment).',
          '',
          '**UI flow:**',
          '1. Payment Details — GET /invoices/{id} (or use invoice from publish). orderId, payNowLabel, lineItems, paymentMethods.',
          '2. Checkout — POST /payments/intent.',
          '3. Success — POST /payments/{id}/confirm (or GET /payments/{id}/receipt).',
          '4. Fail — POST /payments/{id}/fail.',
          '',
          '**Invoice labels:** Service Charge, Platform Fee, Trader Offer, Promo Code.',
          '**Success:** title Payment Successful!, timeline Paid → Confirmed → Service.',
          '**Fail:** title Payment Failed + retryPayment action.',
          '',
