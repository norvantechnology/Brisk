  ...
  definition: {
    ...
    info: {
      ...
      description: [
        ...
        '- **Direct Trader Offer → Job → Pay:** claim offer → POST /jobs → PUT /jobs/{id}/location → POST /jobs/{id}/publish → GET /invoices/{id} → POST /payments/intent → POST /payments/{id}/confirm → GET /bookings/{id}',
        '- **Customer / Jobs:** /jobs (draft, location, publish). **Customer / Checkout:** /invoices, /payments, /bookings',
        '',
    ...
    tags: [
      ...
      },
      {
        ...
        description:
          'Mobile Offers tab: GET /trader-offers, GET /brisk-offers, POST .../claim, promo codes. Auth: customer Bearer token. Filter params map 1:1 to the filter modal. Claim returns nextJobPrefill for Post a New Job.',
      },
      {
        name: 'Customer / Jobs',
        description: [
          'Customer Direct Trader / Post a New Job flow.',
          '',
          '**Auth:** Customer Bearer on every endpoint.',
          '',
          '**Endpoints:** POST/GET /jobs · GET/PATCH /jobs/{id} · PUT /jobs/{id}/location · POST /jobs/{id}/publish',
          '',
          '**Typical order:** claim offer → create draft (appliedTraderOfferId + claimId + traderId + serviceCharge) → set location → publish → checkout.',
          '',
          '**Publish (Direct Trader):** creates Quote (ACCEPTED) + Booking + Invoice (Service Charge, Platform Fee 10% post-offer, Trader Offer discount). Response includes full invoice for Payment Details.',
          '',
          'Schemas: Job, CreateJobRequest, PublishJobResponse under Components.',
        ].join('\n'),
      },
      {
        name: 'Customer / Checkout',
        description: [
          'Invoice, payment, and booking APIs for Payment Details + Payment Successful screens.',
          '',
          '**Auth:** Customer Bearer (must own the booking/invoice/payment).',
          '',
          '**Endpoints:** GET /invoices/{id} · POST /invoices/{id}/apply-promo · POST /payments/intent · POST /payments/{id}/confirm · GET /payments/{id}/receipt · GET /bookings/{id}',
          '',
          '**Invoice UI:** orderId, payNowLabel, lineItems (Service Charge / Platform Fee / Trader Offer / Promo Code), serviceSummary, paymentMethods.',
          '',
          '**Receipt UI:** title, transactionId, amountPaid, timeline Paid → Confirmed → Service.',
          '',
          '**Payments:** currently mock Stripe (mock: true). Confirm without real Stripe SDK.',
          '',
          'Schemas: Invoice, PaymentIntentResponse, PaymentReceipt, BookingDetail under Components.',
        ].join('\n'),
      },
      {
