...
const options: swaggerJSDoc.Options = {
  definition: {
    ...
    tags: [
      ...
      { name: 'Mobile / Auth', description: 'Customer & Trader auth. Traders need extra email verify (`POST /auth/verify-email`) before onboarding.' },
      {
        name: 'Trader / Onboarding',
        description: [
          'Trader-only step-by-step onboarding (Sole `SOLO` & Company `COMPANY`). Separate from Customer registration.',
          '',
          '**Prerequisites (Mobile / Auth):** `POST /auth/register` (role=TRADER) → `POST /auth/verify-otp` → `POST /auth/verify-email`',
          '',
          '**Figma flow:** Business Type → Entity Documents → Select Categories → Category Documents → Personal/Company Info → Bank (or skip) → Service Radius → Submit',
          '',
          '**Auth:** Bearer access token on every endpoint. Test OTP: `123456`.',
          '',
          '**Categories list:** `GET /categories` (Mobile / Categories tag).',
        ].join('\n'),
      },
